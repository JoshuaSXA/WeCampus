...
class CarPoolingController {
    ...
    public function createComplaintCase() {

        $carPoolingID = $_REQUEST['carpool_id'];

        $defendant = $_REQUEST['defendant'];

        $descrText = $_REQUEST['descr'];

        $sql = "INSERT INTO carpool_complaint (accuser, defendant, carpool_id, descr, time) VALUES ((?), (?), (?), (?), NOW())";
        ...
        if(mysqli_stmt_prepare($stmt, $sql)){
            ...
            // 绑定参数
            mysqli_stmt_bind_param($stmt, "ssis", $this->openID, $defendant, $carPoolingID, $descrText);
